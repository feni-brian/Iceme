...
enum SocialFeedTag: Identifiable {
    ...
    var id: String {
        switch self {
        case .title(let string): return "tag title: \(string)"
        case .donut(let donut): return "tag donut: \(donut.id)"
        case .city(let city): return "tag city: \(city.id)"
        }
    }
    ...
    var title: LocalizedStringKey {
        ...
        case .donut(let donut): return .init(donut.name)
        case .city(let city): return .init(city.name)
        }
    }
...
extension SocialFeedPost {
    ...
    static let additionalContent: [SocialFeedPost] = {
        var date = Date.now
        func olderDate() -> Date {
            // Minutes in the past
            date = date.addingTimeInterval(-60 * .random(in: 5...30))
        ...
        return [
            ...
            SocialFeedPost(
                ...
                date: olderDate(),
                tags: [.title("Carrot"), .title("Carrots of Social Feed"), .title("Bunnies of Social Feed"), .title("Carrots"), .title("Donuts for Bunnies")]
            )
        ]
    }()
}
