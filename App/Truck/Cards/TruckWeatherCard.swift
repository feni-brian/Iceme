...
struct TruckWeatherCard: View {
    ...
    var headerUseNavigationLink: Bool = false
    ...
    var body: some View {
        VStack {
            CardNavigationHeader(panel: .city(City.sanFrancisco.id), navigation: navigation, label: { Label("Forecast", systemImage: "cloud.sun") })
            chart
                .frame(minHeight: 180)
        }
        .padding(10)
        ...
        .task {
            do {
                ...
                forecast = TruckWeatherForecast(entries: weather.map {
                    .init(date: $0.date, degrees: $0.temperature.converted(to: .celsius).value, isDaylight: $0.isDaylight)
                })
            } catch {
    ...
    var chart: some View {
        Chart {
            ...
            ForEach(forecast.nightTimeRanges, id: \.lowerBound) { range in
                RectangleMark(xStart: .value("Hour", range.lowerBound), xEnd: .value("Hour", range.upperBound))
                    .opacity(0.5)
                    .mask(content: { areaMarks(seriesKey: "Mask", value: range.lowerBound.timeIntervalSince1970) })
                
                if range.lowerBound != forecast.entries.first!.date {
                    let date = range.lowerBound
                    RectangleMark(x: .value("Date", date), yStart: .value("Temperature", forecast.low - 0.5), yEnd: .value("Temperature", forecast.temperature(at: date) + 0.5), width: .fixed(4))
        ...
        .chartYAxis {
            AxisMarks(values: .automatic(minimumStride: 5, desiredCount: 6, roundLowerBound: false)) { value in
                AxisValueLabel("\(value.as(Double.self)!.formatted())ºC")
                AxisTick()
                AxisGridLine()
            }
        }
    ...
    static var placeholderForecast: TruckWeatherForecast {
        func entry(hourOffset: Int, degrees: Double, isDaylight: Bool) -> TruckWeatherForecast.WeatherEntry {
            let startDate = Calendar.current.date(from: DateComponents(year: 2022, month: 1, day: 1, hour: 1))!
...
struct TruckWeatherForecast {
    ...
    var low: Double { return entries.map(\.degrees).min()! - 2 }
    var hottestEntry: WeatherEntry { return entries.sorted { $0.degrees > $1.degrees }.first! }
    ...
    var nightTimeRanges: [Range<Date>] {
        ...
        if let lowerBound = currentLowerBound { results.append(lowerBound..<entries.last!.date) }
        return results
    }
    
    var binRange: ClosedRange<Date> {
        let startDate: Date = entries.map(\.date).first(where: { Calendar.current.component(.hour, from: $0).isMultiple(of: 3) })!
        let endDate: Date = entries.map(\.date).reversed().first(where: { Calendar.current.component(.hour, from: $0).isMultiple(of: 3) })!
