...
struct OrderCompleteView: View {
    var order: Order
    var currentAppVersion: String? { Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String }
    ...
    var shouldRequestReview: Bool {
        ...
        guard let currentAppVersion = self.currentAppVersion else { return false }
        let dateLastAsked = Date(timeIntervalSince1970: datePromptedForReview)
        var dateComponent = DateComponents()
        dateComponent.month = 4
        guard let fourMonthsLater = Calendar.current.date(byAdding: dateComponent, to: dateLastAsked) else { return false }
