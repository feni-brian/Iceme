...
struct TopDonutSalesChart: View {
    ...
    var sortedSales: [DonutSales] { sales.sorted() }
    var totalSales: Int { sales.map(\.sales).reduce(0, +) }
    ...
    var chart: some View {
        Chart {
            ForEach(sortedSales) { sale in
                BarMark(x: .value("Donut", sale.donut.name), y: .value( "Sales", sale.sales))
    ...
    func donutFromAxisValue(for value: AxisValue) -> Donut {
        guard let name = value.as(String.self) else { fatalError("Could not convert axis value to expected String type") }
        guard let result = sales.first(where: { $0.donut.name == name }) else { fatalError("No top performing DonutSales with given donut name: \(name)") }
        return result.donut
    }
}
