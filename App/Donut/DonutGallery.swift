...
struct DonutGallery: View {
    ...
    var filteredDonuts: [Donut] { model.donuts(sortedBy: sort).filter { $0.matches(searchText: searchText) }}
    ...
    var body: some View {
        ...
        .toolbarRole(.browser)
        #endif
        .toolbar { ToolbarItemGroup { toolbarItems } }
    ...
    var toolbarItems: some View {
        ...
        Menu {
            ...
            Picker("Sort", selection: $sort) {
                Label("Name", systemImage: "textformat").tag(DonutSortOrder.name)
                Label("Popularity", systemImage: "textformat").tag(DonutSortOrder.popularity(popularityTimeframe))
                Label("Flavour", systemImage: "textformat").tag(DonutSortOrder.flavor(sortFlavor))
            }
            .pickerStyle(.inline)
            ...
            if case .popularity = sort {
                ...
                Picker("Flavour", selection: $sortFlavor) {
                    ForEach(Flavor.allCases) { flavour in
                        Text(flavour.name).tag(flavour)
                    }
                }
                .pickerStyle(.inline)
            }
        } label: {
            Label("Layout Options", systemImage: layout.imageName).labelStyle(.iconOnly)
        }
    }
}

struct DonutGallery_Previews: PreviewProvider {
    struct Preview: View {
