//
//  DonutGalleryGrid.swift
//  Iceme Corner
//
//  Created by Feni Brian on 22/07/2023.
//

import SwiftUI
import FoodTruckKit

struct DonutGalleryGrid: View {
    var donuts: [Donut]
    var width: Double
    
    #if os(iOS)
    @Environment(\.horizontalSizeClass) private var sizeClass
    #endif
    @Environment(\.dynamicTypeSize) private var dynamicSize
    
    var useReducedThumbnailSize: Bool {
        #if os(iOS)
        if sizeClass == .compact { return true }
        #endif
        if dynamicSize >= .xxxLarge { return true }
        #if os(iOS)
        if width <= 390 { return true }
        #elseif os(macOS)
        if width <= 520 { return true }
        #endif
        return false
    }
    
    var cellSize: Double { useReducedThumbnailSize ? 100 : 150 }
    var gridItems : [GridItem] { [GridItem(.adaptive(minimum: cellSize), spacing: 20, alignment: .top)] }
    
    var thumbnailSize: Double {
        #if os(iOS)
        return useReducedThumbnailSize ? 60 : 100
        #else
        return useReducedThumbnailSize ? 40 : 80
        #endif
    }
    
    var body: some View {
        LazyVGrid(columns: gridItems, spacing: 20) {
            ForEach(donuts) { donut in
                NavigationLink(value: donut.id) {
                    VStack {
                        DonutView(donut: donut)
                            .frame(width: thumbnailSize, height: thumbnailSize)
                        VStack {
                            let flavour = donut.flavors.mostPotentFlavor
                            Text(donut.name)
                            HStack(spacing: 4) {
                                flavour.image
                                Text(flavour.name)
                            }
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                        }
                        .multilineTextAlignment(.center)
                    }
                }
                .buttonStyle(.plain)
            }
        }
        .padding()
    }
}

struct DonutGalleryGrid_Previews: PreviewProvider {
    struct Preview: View {
        @State private var donuts = Donut.all
        var body: some View {
            GeometryReader { geometryProxy in
                ScrollView(content: { DonutGalleryGrid(donuts: donuts, width: geometryProxy.size.width) })
            }
        }
    }
    static var previews: some View {
        Preview()
    }
}
