...
struct RefundView: View {
    ...
    @State private var refundSheetIsPresented: Bool = false
    ...
    var body: some View {
        ...
        .task { @MainActor in
            for await transaction in StoreKit.Transaction.all {
                ...
                if transaction.unsafePayloadValue.isRevoked { continue }
                recentTransactions.append(transaction.unsafePayloadValue)
                if recentTransactions.count >= 10 { break }
            }
        }
        .refundRequestSheet(for: selectedTransactionID ?? 0, isPresented: $refundSheetIsPresented) { result in
...
struct TransactionRowView: View {
    ...
    var title: String { product?.displayName ?? transaction.productID }
    var subtitle: String {
        let purchaseDate = transaction.purchaseDate.formatted(date: .long, time: .omitted)
