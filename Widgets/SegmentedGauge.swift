...
struct SegmentedGauge<Label: View>: View {
...
struct SegmentedGaugeStyle: GaugeStyle {
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: metrics.gap) {
                ForEach(0..<total, id: \.self) { index in
                    Segment(metrics, position(for: index, with: value, in: total))
    ...
    struct Segment: View {
        ...
        init(_ metrics: Metrics, _ position: Position) {
        ...
        struct Segment: Shape {
            ...
            func corners(for rect: CGRect) -> (topRight: CGFloat, topLeft: CGFloat, bottomRight: CGFloat, bottomLeft: CGFloat) {
                ...
                let position: Position = {
                    ...
                    if layoutDirection == .rightToLeft {
                        ...
                        else if position == .trailing { position = .leading }
                    }
                    return position
                }()
                
                switch position {
                case .leading: return (rectCorner, rectCorner, capsuleCorner, capsuleCorner)
                case .trailing: return (capsuleCorner, capsuleCorner, rectCorner, rectCorner)
                case .middle: return (rectCorner, rectCorner, rectCorner, rectCorner)
                case .single: return (capsuleCorner, capsuleCorner, capsuleCorner, capsuleCorner)
                }
            }
        }
    }
