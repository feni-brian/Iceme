...
struct OrdersWidget: Widget {
    ...
    struct Provider: TimelineProvider {
        ...
        func getSnapshot(in context: Context, completion: @escaping (OrdersWidget.Entry) -> Void) { completion(.preview) }
        
        func getTimeline(in context: Context, completion: @escaping (Timeline<OrdersWidget.Entry>) -> Void) {
            var entries: [Entry] = []
...
struct OrdersWidgetView: View {
    ...
    var body: some View {
        ...
        case .systemSmall:
            ZStack {
                ...
                VStack(alignment: .leading) {
                    ...
                    Gauge(value: Double(entry.orders), in: 0 ... Double(entry.quota), label: { EmptyView() }) {
        ...
        case .accessoryCircular:
            Gauge(value: Double(entry.orders), in: 0 ... Double(entry.quota), label: { Image.donutSymbol }) {
